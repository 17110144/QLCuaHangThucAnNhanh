...
namespace FastFood.UCFunction
{
    public partial class UC_FoodManager : UserControl
    {
        ...
        private void btnCapNhat_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (check == 1)
                {
                    ...
                    bool addProduct = lib.cls_Product._addProduct(1, genMaSP, txtTenMonAn.Text, Convert.ToInt32(cmbNhaCungCap.SelectedValue), int.Parse(txtGiaTien.Text), int.Parse(txtGiamGia.Text), int.Parse(txtSoLuong.Text));
        ...
        {
            dtListProduct.DataSource = lib.cls_Product._searchProduct(1, txtTimKiem.Text, cmbFilter.SelectedValue.ToString());
            _formatDT();
        }
